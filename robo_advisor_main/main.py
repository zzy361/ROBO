...
try:
    ...
    trade_day = (datetime.today().date() - relativedelta(days=90)).strftime('%Y%m%d')
    ...
    for tmp in today_json:
        tmp['data_date']=str(datetime.today().strftime("%Y-%m-%d 00:00:00"))
        tmp['creat_date']=str(datetime.today().strftime("%Y-%m-%d %H:%M:%S"))

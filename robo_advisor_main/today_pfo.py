...
for poc in list(map(lambda x: 'ft' + x, [str(i) for i in list(range(1, 10))])):
    ...
    dict.append({"recomm_guid": str(recomm_guid),"data": {"recomm_guid": str(recomm_guid), "data_date": str(data_date), "roi": roi, "risk": risk, "cp": cp, "rr": rr, "creat_date": str(create_date),"funds": dict1, "note": note_msg}})
    j += 1

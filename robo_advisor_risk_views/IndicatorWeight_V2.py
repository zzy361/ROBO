...
class  LongShort_Indicator():
    def __init__(self,dict_inpui):
        self.conn = dict_inpui['conn']
        self.alpha=dict_inpui['alpha']
        self.pomax=dict_inpui['pomax']
        self.pomin=dict_inpui['pomin']
        self.npow=dict_inpui['npow']
        query='''
        ...
        '''
        data_TradingDay=pd.read_sql(query,conn)
    ...
    def get_oppsrisk_level(self,result_weight):
        ...
        result_risk_mix['Strategy']="LongShortIndicator_mix"
        ...
        result_risk_mix['risk_comment']='Tony'
        result_risk_mix=pd.merge(result_risk_mix,self.data_asset,on=['risk_date','asset_benchmark'],how='left')
        return result_risk_mix[['risk_date','asset_name','asset_benchmark','risk','data_source','risk_comment']]

    def get_data_percentile(self):
        query_percent = '''SELECT * FROM jf_research_quant.assetquantile_1month'''
        data_percentile=pd.read_sql(query_percent,conn)
    ...
    def get_data_realandpre(self,data_percentile):
        ...
        data_asset =df_pre[["risk_date","asset_name","asset_benchmark"]].copy()
        data_asset.drop_duplicates(["risk_date","asset_name","asset_benchmark"],inplace=True)
...
if __name__ == '__main__':
    conn = create_engine( 'mysql+pymysql://tommy:tommy@wei@rm-uf679020c6vrt28in7o.mysql.rds.aliyuncs.com:3306/jf_data?charset=utf8')
    list_Strategy=['AI', '量化多因子', '吉富投资',  'AI预测算法']#$['AI', '量化多因子', '吉富投资', '吉富多空系数', 'AI预测算法']
    dict_input={"conn":conn,"alpha":0.2,"pomax":1,"pomin":0.0,'npow':1.1}
    LSIndicator = LongShort_Indicator( dict_input)

    data_percentile = LSIndicator.get_data_percentile()
    data_real, data_predict = LSIndicator.get_data_realandpre(data_percentile)
    data_predict = data_predict[data_predict['Strategy'].isin(list_Strategy)].copy(0)
    data_realpercentile= LSIndicator.map_pencentile(data_real, data_percentile)
    result_weight = LSIndicator.get_oppsweight(data_predict, data_realpercentile)
    result_weight = LSIndicator.risk_polimit(result_weight)
    result_risk_mix = LSIndicator.get_oppsrisk_level(result_weight)
    df1=result_weight[result_weight['benchmark']=='SPX Index'].copy()
    pvt_df1=df1.pivot(columns='Strategy',index='nav_date',values='Weight')

    df2=result_risk_mix[result_risk_mix['asset_benchmark']=='SPX Index'].copy()
    df3=data_realpercentile[data_realpercentile["benchmark"]=='SPX Index'].copy()
    df3.rename(columns={"nav_date":"risk_date"},inplace=True)
    df_compair=pd.merge(df2[['risk_date','risk']],df3[['risk_date','Indicator_real']],how='inner',on='risk_date')
    df_compair.set_index('risk_date',inplace=True)

    mpl.rcParams['font.sans-serif'] = ['SimHei']
    pvt_df1.plot(title=str(dict_input['alpha'])+'_'+str(dict_input['npow']))
    df_compair.plot(title=str(dict_input['alpha'])+'_'+str(dict_input['npow']))
    print(dict_input['alpha'],dict_input['npow'])

    df1=data_predict[(data_predict['benchmark']=='SPX Index' )& (data_predict['Strategy']=='AI' )][['nav_date','Sign']]
    df2=data_predict[(data_predict['benchmark']=='SPX Index' )& (data_predict['Strategy']=='量化多因子' )][['nav_date','Sign']]
    df3 = data_predict[(data_predict['benchmark'] == 'SPX Index') & (data_predict['Strategy'] == '吉富投资')][['nav_date', 'Sign']]
    df1.rename(columns={"Sign":"AI"},inplace=True)
    df2.rename(columns={"Sign": "量化多因子"}, inplace=True)
    df3.rename(columns={"Sign": "吉富投资"}, inplace=True)

    df=df_compair.copy()
    df.reset_index(inplace=True)
    df.rename(columns={"risk_date":"nav_date"},inplace=True)
    df=pd.merge(df,df1,on='nav_date',how='left')
    df = pd.merge(df, df2, on='nav_date', how='left')
    df = pd.merge(df, df3, on='nav_date', how='left')
    df.set_index('nav_date',inplace=True)
    df.plot(title=str(dict_input['alpha'])+'_'+str(dict_input['npow']))

    pvt_df1['comment'] = list(map(lambda x: 'AI_'+str(round(x,2)), pvt_df1['AI']))
    pvt_df1['comment'] = list(map(lambda x,y:x+ '__量化多因子_' + str(round(y,2)), pvt_df1['comment'],pvt_df1['量化多因子']))
    pvt_df1['comment'] = list(map(lambda x, y: x + '__吉富投资_' + str(round(y, 2)), pvt_df1['comment'], pvt_df1['吉富投资']))
    pvt_df1.reset_index(inplace=True)
    pvt_df1.rename(columns={'nav_date':'risk_date'},inplace=True)
    df_result=pd.merge(result_risk_mix,pvt_df1[['risk_date','comment']],on='risk_date',how='left')

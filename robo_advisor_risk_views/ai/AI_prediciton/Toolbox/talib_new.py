...
def ADX(df, n, n_ADX, ksgn='close'):
    ...
    ATR = pd.Series((TR_s.rolling(window=n).mean()))

    UpI = pd.Series(UpI)
    DoI = pd.Series(DoI)

    PosDI = pd.Series((UpI.rolling(window=n).mean()) / ATR)
    NegDI = pd.Series((DoI.rolling(window=n).mean()) / ATR)
    ds = pd.Series(((abs(PosDI - NegDI) / (PosDI + NegDI)).rolling(window= n_ADX).mean()), name=xnam)
    ds.index = df.index
    df[xnam] = ds
...
def CCI(df, n, ksgn='close'):
    ...
    CCI = pd.Series((PP - PP.rolling(window=n).mean() ) / (PP.rolling(window=n).std()), name=xnam)
...
　　估波指标由Edwin·Sedgwick·Coppock于1962年提出，主要用于判断牛市的到来。
    ...
    Copp = pd.Series((ROC1 + ROC2).rolling(window=n).mean(), name=xnam)
...
　　佳庆指标（CHAIKIN）是由马可·蔡金（Marc Chaikin）提出的，聚散指标（A/D）的改良版本。
    ...
    Chaikin = pd.Series(ad.rolling(window= 3).mean() - ad.rolling(window = 9).mean(), name=xnam)
...
def EMA(df, n, ksgn='close'):
    ...
    EMA = pd.Series(df[ksgn].rolling(window=n).mean(), name=xnam)
...
def EOM(df, n):
   ...
   由于股价的变化和成交量的变化都可以引发该指标数值的变动,因此,EMV实际上也是一个量价合成指标。
    ...
    Eom_ma = pd.Series(EoM.rolling(window=n).mean(), name=xnam)
...
　　肯特纳通道（KC）是一个移动平均通道，由叁条线组合而成(上通道、中通道及下通道)。
    ...
    kcm = (df['high'] + df['low'] + df[ksgn]) / 3
    kcu = (4 * df['high'] - 2 * df['low'] + df[ksgn]) / 3
    kcd = (-2 * df['high'] + 4 * df['low'] + df[ksgn]) / 3

    KelChM = pd.Series(kcm.rolling(window = n).mean(), name=xnam)
    KelChU = pd.Series(kcu.rolling(window = n).mean(), name=xnam2)
    KelChD = pd.Series(kcd.rolling(window = n).mean(), name=xnam3)

...
　　肯特纳通道（KC）是一个移动平均通道，由叁条线组合而成(上通道、中通道及下通道)。
    ...
    KelChU = pd.Series(pd.rolling_mean((4 * df['high'] - 2 * df['low'] + df[ksgn]) / 3, n), name=xnam2)
    KelChD = pd.Series(pd.rolling_mean((-2 * df['high'] + 4 * df['low'] + df[ksgn]) / 3, n), name=xnam3)
    df = df.join(KelChM)
    df = df.join(KelChU)
    df = df.join(KelChD)
...
　　MASS指标是所有区间震荡指标中，风险系数最小的一个。		
    ...
    Mass = EX1 / EX2
    MassI = pd.Series(pd.rolling_sum(Mass, 25), name=xnam)
    df = df.join(MassI)
    return df
...
　　经过一连串参数顺化的过程后，UOS指标比一般单一参数的振荡指标，更能够顺应各种不同的市况。
    ...
    while i < len(df) - 1:
        ...
        BP_l.append(BP)
        i = i + 1
    UltO = pd.Series((4 * pd.rolling_sum(pd.Series(BP_l), 7) / pd.rolling_sum(pd.Series(TR_l), 7)) + (2 * pd.rolling_sum(pd.Series(BP_l), 14) / pd.rolling_sum(pd.Series(TR_l), 14)) + (pd.rolling_sum(pd.Series(BP_l), 28) / pd.rolling_sum(pd.Series(TR_l), 28)), name=xnam)
